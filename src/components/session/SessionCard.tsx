...
const SessionCard: React.FC<SessionCardProps> = ({ 
  ...
  const ownerName = session.ownerName || 'Unknown User';
  const ownerPhotoURL = session.ownerPhotoURL || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(ownerName);
  const thumbnailUrl = session.thumbnailUrl || `https://picsum.photos/seed/${session.id}/320/180`;
        ...
        <div className="flex justify-between items-center mt-4">
          {isOwner ? (
            <div className="flex space-x-2">
              <button 
                onClick={() => onUpdatePrivacy(session.id, !session.isPublic)}
              ...
              >
                {session.isPublic ? <Unlock className="h-4 w-4" /> : <Lock className="h-4 w-4" />}
              </button>
              <button 
